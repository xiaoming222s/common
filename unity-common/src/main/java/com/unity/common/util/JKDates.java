...
public final class JKDates {
    private static final Logger logger = LoggerFactory.getLogger(JKDates.class);

    private static final Logger LOG = LoggerFactory.getLogger(JKDates.class);
    ...
    public static Date date(Integer... fields) {
        ...
        for (int i = 0; i < FIELDS.length; ++i) {
            int val = (Integer) JKArrays.get(fields, i, 0);
            if (val > 0 && i == 1) {
                --val;
            }
    ...
    public static List<String> getMonthListByStartDate2EndDate(String startDate, String endDate) {
        ...
        try {
            //进行日期转换
            Calendar startDateCal = DateUtils.toCalendar(DateUtils.parseDate(startDate, "yyyy-MM-dd"));
            Calendar endDateCal = DateUtils.toCalendar(DateUtils.parseDate(endDate, "yyyy-MM-dd"));
    ...
    public static List<String> converseDate(String date, int range) throws ParseException {
        Calendar calendar = DateUtils.toCalendar(DateUtils.parseDate(date, "yyyy-MM"));
