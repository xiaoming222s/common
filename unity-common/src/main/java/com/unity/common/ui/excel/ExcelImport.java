...
@Slf4j
public class ExcelImport extends ImportBaseService {
    ...
    private <T> List<T> importExcel(Collection<T> result, Sheet sheet, Class<?> pojoClass,
        ...
        return importExcel(result,sheet,pojoClass,excelParams,params,pictures);
//        List collection = new ArrayList();
//        Map<String, ExcelImportEntity> excelParams = new HashMap<String, ExcelImportEntity>();
//        List<ExcelCollectionParams> excelCollection = new ArrayList<ExcelCollectionParams>();
//        String targetId = null;
...
//            getAllExcelField(targetId, fileds, excelParams, excelCollection, pojoClass, null, null);
//        }
//        System.out.println("excelParams=="+ JSON.toJSONString(excelParams));
//        Iterator<Row> rows = sheet.rowIterator();
//        for (int j = 0; j < params.getTitleRows(); j++) {
//            rows.next();
//        }
...
//        return collection;
     ...
     */
    public ExcelImportResult importExcelByIs(Map<String, ExcelImportEntity> columns,
                                             InputStream inputstream, Class<?> pojoClass,
        ...
        for (int i = params.getStartSheetIndex(); i < params.getStartSheetIndex()
            ...
            result.addAll(importExcel(result,book.getSheetAt(i),pojoClass,columns,params,pictures));
//            result.addAll(importExcel(result, book.getSheetAt(i), pojoClass, params, pictures));
        ...
        if (needMore) {
            ...
            importResult.setFailWorkbook(book);
        ...
        if (params.getImportFields() != null) {
            if (params.isNeedCheckOrder()) { // 同时校验列顺序
                ...
                if (params.getImportFields().length != titlemap.size()) {
                    ...
                    throw new ExcelImportException(ExcelImportEnum.IS_NOT_A_VALID_TEMPLATE.getMsg()+"【excel列顺序不一致】");
                }
                int i = 0;
            ...
            for (ExcelImportEntity excelImportEntity : collection) {
                if (excelImportEntity.isImportField()
                    ...
                    throw new ExcelImportException(ExcelImportEnum.IS_NOT_A_VALID_TEMPLATE.getMsg()+"【"+excelImportEntity.getName() + "必须有,但是没找到】");
                }
            }
